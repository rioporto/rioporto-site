...
import { MessageCircle, Send, User, AlertCircle, Lock } from "lucide-react"
...
interface BlogCommentsProps {
  postId: string
  requireLoginToView?: boolean // Nova prop para controlar visibilidade
}

export function BlogComments({ postId, requireLoginToView = false }: BlogCommentsProps) {
  ...
  useEffect(() => {
    checkUser()
    // Sempre carrega comentários, independente de estar logado
    if (!requireLoginToView || user) {
      loadComments()
    } else {
      setLoading(false)
    }
  }, [postId, user, requireLoginToView])
  ...
  // Se requireLoginToView está ativo e usuário não está logado
  if (requireLoginToView && !user) {
    return (
      <Card>
