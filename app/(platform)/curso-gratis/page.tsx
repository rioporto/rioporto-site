...
export default function CursoGratisPage() {
  ...
  // Tracking - usando email do usuário como identificador
  const { 
    trackPageView, 
    trackCourseComplete,
    getProgress 
  } = useMinicursoTracking({ 
    token: user?.email || '', // Usar email como token para usuários logados
  ...
  useEffect(() => {
    ...
    loadProgress();
  }, [user, router, getProgress]);

  // Rastrear mudança de página
  useEffect(() => {
    if (user && chapters[currentPage]) {
  ...
  const handleDownload = async () => {
    ...
    const response = await fetch('/api/minicurso/generate-download-link', {
      method: 'POST',
      headers: {
